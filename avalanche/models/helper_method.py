...
class MultiTaskDecorator(MultiTaskModule):
    ...
    def __init__(self, model: nn.Module, classifier_name: str, masking: bool):
        """
        ...
        setattr(self, classifier_name, MultiHeadClassifier(in_size, out_size,masking=masking))
    ...
    def forward_two_task(self, x: torch.Tensor, task_label: list):
        out = self.model(x)
        assert len(task_label) == 2
        out1 = getattr(self, self.classifier_name)(
            out.view(out.size(0), -1), task_labels=task_label[0]
        )
        out2 = getattr(self, self.classifier_name)(
            out.view(out.size(0), -1), task_labels=task_label[1]
        )
...
def as_multitask(model: nn.Module, classifier_name: str, masking: bool) -> MultiTaskModule:
    ...
    """
    return MultiTaskDecorator(model, classifier_name, masking=masking)
