...
class ICaRL(SupervisedTemplate):
    ...
    def __init__(
        ...
        icarl = _ICaRLPlugin(mixup, memory_size, buffer_transform, fixed_memory)
...
class _ICaRLPlugin(SupervisedPlugin):
    ...
    def __init__(self, one_hot, memory_size, buffer_transform=None, fixed_memory=True):
        """
        ...
        if strategy.clock.train_exp_counter != 0:
            ...
            if self.one_hot:
                ...
                memory = make_tensor_classification_dataset(
                    ...
                    torch.cat(self.onehot_memory).cpu(),
                    transform=self.buffer_transform,
                    target_transform=None,
                )
                adapt_d = MDataset([strategy.adapted_dataset, memory])
                strategy.adapted_dataset = make_tensor_classification_dataset(adapt_d)

            # strategy.adapted_dataset = MixupDataset(strategy.adapted_dataset, memory)
            else:
                memory = make_tensor_classification_dataset(
                    torch.cat(self.x_memory).cpu(),
                    torch.tensor(
    ...
    def construct_exemplar_set(self, strategy: SupervisedTemplate):
        ...
        for iter_dico in range(nb_cl):
            ...
            one_hots = []
            if self.one_hot:
                for idx, (class_pt, _,onehot, _) in enumerate(eval_dataloader):
                ...
                one_hots = torch.cat(one_hots,dim=0)
            else:
            ...
            """是否mixup，决定是否要注释"""
            if self.one_hot:
                self.onehot_memory.append(one_hots[torch.where(pick == 1)[0]])
    ...
    def reduce_exemplar_set(self, strategy: SupervisedTemplate):
        ...
        for i in range(len(self.x_memory) - nb_cl[tid]):
            ...
            if len(self.onehot_memory) != 0:
                self.onehot_memory[i] = self.onehot_memory[i][torch.where(pick == 1)[0]]
