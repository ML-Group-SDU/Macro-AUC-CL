...
# _default_collate_mbatches_fn = None
...
# TaskBalanced 调用了  GroupBalanced
...
class TaskBalancedDataLoader:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._dl = GroupBalancedDataLoader(datasets=task_datasets,
                                           mixup=self.mixup,
...
class GroupBalancedDataLoader:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if "collate_fn" in kwargs:
            ...
            if self.mixup == "first_exp_mixup":
                self.collate_fn = traditional_mixup
            else:
    ...
    def __iter__(self):
        ...
        for it in range(max_num_mbatches):
            ...
            if self.mixup == 'first_exp_mixup':
                yield self.collate_fn(mb_curr, self.mix_times, self.class_num)
            else:
...
class ReplayDataLoader: # Ori replay dataloader
    ...
    def __init__(
        ...
        **kwargs
        ...
        if "collate_fn" in kwargs:
            ...
            self.collate_fn = self.data.collate_fn if self.mixup=="no" else mixup_classification_collate_mbatches_fn
        ...
        做加权
        """
        if self.mixup != 'no' and self.mu > 0:
            print("self.mixup: ", self.mixup)
            print("self.mu: ", self.mu)
            self.memory_batch_sizes, _ = self._get_weighted_batch_sizes(memory, batch_size_mem, remaining_example, self.mu, similarity_order)
            print("memory batch size:", self.memory_batch_sizes)
        else:
    ...
    def __iter__(self):
        loader_data, sampler_data = self._create_loaders_and_samplers(
            self.data, self.data_batch_sizes,no_worker=False
        )

        loader_memory, sampler_memory = self._create_loaders_and_samplers(
            self.memory, self.memory_batch_sizes,no_worker=True
        )

        # 自己加的
        # task_list = self.memory.targets_task_labels
        # task_l_set = set(task_list)
        # task_count_dict = {}
        # for i in task_l_set:
        #     task_count_dict[i] = task_list.count(i)
        ...
        try:
            for it in range(max_len):
                ...
                if self.mixup == 'no':
                    ...
                    yield self.collate_fn(mb_curr,self.mixup, self.mix_times, self.class_num, self.scenario_mode)
        except StopIteration:
            return
    ...
    def _create_loaders_and_samplers(self, data, batch_sizes,no_worker):
        loaders = dict()
        samplers = dict()
...
class WReplayDataLoader: # Weighted REplay Dataloader
    """Custom data loader for rehearsal/replay strategies."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.oversample_small_tasks = oversample_small_tasks
        self.task_balanced_dataloader = task_balanced_dataloader
        self.collate_mbatches = collate_mbatches if self.mixup == 'no' else mixup_classification_collate_mbatches_fn
        self.data_batch_sizes: Union[int, Dict[int, int]] = dict()
        self.memory_batch_sizes: Union[int, Dict[int, int]] = dict()
        self.distributed_sampling = distributed_sampling
        self.loader_kwargs = kwargs
    ...
    def __iter__(self):
        ...
        try:
            for it in range(max_len):
                ...
                if self.mixup == 'no':
                    yield self.collate_mbatches(mb_curr, self.similarity_order, self.mu)

                else:
                    yield self.collate_mbatches(mb_curr, self.mix_times, self.class_num, self.scenario_mode,self.similarity_order,self.mu)
        except StopIteration:
            return
