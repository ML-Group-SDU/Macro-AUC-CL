...
def SplitTinyImageNet(
    ...
    *,
    ...
    train_set, test_set = _get_tiny_imagenet_dataset(dataset_root, balance)

    return nc_benchmark(
        train_dataset=train_set,
        test_dataset=test_set if not train_error else train_set,
...
def _get_tiny_imagenet_dataset(dataset_root, balance=True):
    if balance:
        train_set = TinyImagenet(root=dataset_root, train=True)

        test_set = TinyImagenet(root=dataset_root, train=False)
    else:
        train_set = SplitTinyImageNet(root=dataset_root, train=True)

        test_set = SplitTinyImageNet(root=dataset_root, train=False)
