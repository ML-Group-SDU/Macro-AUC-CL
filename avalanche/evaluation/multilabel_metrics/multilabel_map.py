...
class Accuracy(Metric[float]):
    ...
    def update(
        ...
        predicted_y, true_y = predicted_y.cpu().numpy(), true_y.cpu().numpy()

        if true_y.shape[1] > 0:
            AP = []
            for i in range(true_y.shape[1]):
                AP.append(average_precision_score(true_y[:,i], predicted_y[:,i]))
...
def eliminate_all_zero_columns(y_true,y_pred):
    ...
    # indictors = torch.any(y_true.bool(),dim=0) # �����1������true���򷵻ص�Ҳ��true�� ����False����ȫ��0

    if indicators.shape[0] > 0:
    # ��Ҫ����ȫΪ0����
        y_true_list = torch.split(y_true,1,1) # ��ÿ���е����ó���
