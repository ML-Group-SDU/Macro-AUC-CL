...
from avalanche.training.plugins import EvaluationPlugin,EarlyStoppingPluginForTrainMetric
from examples.utils.utils import fix_seeds
from tools.SAM.solver.build import build_optimizer
from tools.myargs import *
...
def main(args):
    ...
    # MODEL CREATION
    model = ResNet18(num_classes=2) if (args.scenario == 'task' or args.scenario == 'domain') else ResNet18(num_classes=10) if args.scenario == 'class' else None
    ...
    cl_strategy = Naive(
        ...
        eval_mb_size=100,
        device=device,
        plugins=[ReplayPlugin(mem_size=2000), sched],
...
if __name__ == "__main__":
    ...
    parser.add_argument('--txtlog_name', type=str, default='train_log.txt')
    ...
    parser.add_argument('--mix_times', type=float, default=0.05)
