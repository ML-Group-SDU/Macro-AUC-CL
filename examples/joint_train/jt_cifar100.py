...
from tools.class_order import cifar100_class_order
...
def main(args):
    ...
    model = ResNet34(num_classes=10) if (args.scenario == 'task' or args.scenario == 'domain') else ResNet34(
        num_classes=100) if args.scenario == 'class' else None

    if args.scenario == 'task':
        model = as_multitask(model, "classifier", masking=False)

    # CL Benchmark Creation
    cifar100 = SplitCIFAR100(n_experiences=10,
                             seed=args.seed,
    ...
    train_stream = cifar100.train_stream
    test_stream = cifar100.test_stream
