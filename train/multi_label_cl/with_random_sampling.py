...
from avalanche.training.plugins import EvaluationPlugin, LRSchedulerPlugin, ERPlugin
...
from avalanche.evaluation.multilabel_metrics.multiLabelLoss import NaiveReweightedLoss,SpecficBCELoss
...
def main(args):
    ...
    if args.opt == "sgd":
        ...
        plugin = [sched,ERPlugin(mem_size=args.mem_size)]
    else:
        plugin = [ERPlugin(mem_size=args.mem_size)]

    # loss function
    if args.crit == "BCE":
        crit = SpecficBCELoss()
    elif args.crit == "ReWeighted":
        crit = NaiveReweightedLoss()
    else:
